...
def main():
	...
	for stock in range(1, nb_stocks+1):
		...
		for dataset in range(1,nb_datasets+1):
			closing_inputs[stock][dataset] = []
			closing_inputs[stock][dataset].append(data[dataset][54,stock-1])
	...
	for stock in range(1,nb_stocks+1):
		...
		# sgd.fit(training_inputs, outputs)

		# logr = LogisticRegression(penalty = 'l1')
		# logr.fit(np.array(training_inputs).reshape((200,1)), np.array(outputs), weights)

		bayR = BayesianRidge(normalize = 'true')
		bayR.fit(training_inputs, outputs)

		# now predict...
		bayR_predict = bayR.predict(prediction_inputs)
		predictions.append([prediction for prediction in bayR_predict])
