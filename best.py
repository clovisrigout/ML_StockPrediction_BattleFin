...
def main():
	...
	for stock in range(1, nb_stocks+1):
		...
		for dataset in range(1,200):
			actual_temp, prices_temp = getData(stock,dataset)
			X.append(prices_temp[-1:])
			Y.append(actual_temp)
		for nb_cv in range(1,10):
			X_train, X_test, y_train, y_test = cross_validation.train_test_split(X,Y, test_size=0.20, random_state=40+nb_cv)
			#find model
			avg_w = perceptronModel(X_train,y_train,0,0)
			print("AVERAGE WEIGHTS")
			print(avg_w)
			cv_weights.append(avg_w)
			#calculate the error of our model... MSE
			mse_s.append(calculateMSE(X_test,y_test,avg_w))
		#take average of mse's
		total_mse = 0
		for x in range(0,len(mse_s)):
			total_mse += mse_s[x]
		avg_mse = float(total_mse)/len(mse_s)
		print("AVERAGE MSE")
		print(avg_mse)
		#if average is ok, then use it for stock prediction. Otherwise use last value.
		if(avg_mse < mse_threshold):
			use_perceptron[stock] = True
		else:
			use_perceptron[stock] = False

		if(True): #actual predictions	
			for dataset in range(201, nb_datasets+1):
				prediction[stock][dataset] = 0
				for i in range(0,len(avg_w)):
					if(use_perceptron[stock]):
						print("USING PERCEPTRON")
						print(avg_w)
						print(prices[stock][dataset][-1])
						prediction[stock][dataset] += prices[stock][dataset][-i-1] * avg_w[i]
